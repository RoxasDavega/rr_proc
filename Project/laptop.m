function varargout = laptop(varargin)
% LAPTOP MATLAB code for laptop.fig
%      LAPTOP, by itself, creates a new LAPTOP or raises the existing
%      singleton*.
%
%      H = LAPTOP returns the handle to a new LAPTOP or the handle to
%      the existing singleton*.
%
%      LAPTOP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in LAPTOP.M with the given input arguments.
%
%      LAPTOP('Property','Value',...) creates a new LAPTOP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before laptop_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to laptop_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help laptop

% Last Modified by GUIDE v2.5 21-May-2022 23:21:33

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @laptop_OpeningFcn, ...
                   'gui_OutputFcn',  @laptop_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before laptop is made visible.
function laptop_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to laptop (see VARARGIN)

% Choose default command line output for laptop
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes laptop wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = laptop_OutputFcn(hObject, eventdata, handles) 
